...
# A mutable table of symbol frequencies. Taking frequency table from neural network
class NeuralFrequencyTable(FrequencyTable):

    # Constructs a frequency table in one of two ways:
    # - NeuralFrequencyTable(freqtable):
    #   Builds a frequency table by copying the given frequency table.
    def __init__(self, freqs):
        if isinstance(freqs, FrequencyTable):
            numsym = freqs.get_symbol_limit()
    ...
    def set(self, symbol, freq):
        ...
        self.frequencies[symbol] = freq
        self.cumulative = None
    #Set frequency to a given frequency from neural net
